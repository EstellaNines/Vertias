...
public class EquipSlot : MonoBehaviour, IDropHandler, ISaveable
{
    ...
    private List<ItemSaveData> GetGridItemsData(BaseItemGrid grid)
    {
        ...
        if (grid != null)
        {
            try
            {
                ...
                Debug.Log($"获取网格物品数据，当前网格中有 {placedItems.Count} 个物品");
                
                foreach (var placedItem in placedItems)
                {
                    if (placedItem.itemObject != null)
                    {
                        var itemComponent = placedItem.itemObject.GetComponent<InventorySystemItem>();
                        if (itemComponent != null)
                        {
                            ItemSaveData itemData = itemComponent.CreateSaveData();
                            itemData.gridPosition = placedItem.position;
                            itemsData.Add(itemData);
                            Debug.Log($"保存物品数据: {itemData.itemDataID} 位置: ({itemData.gridPosition.x}, {itemData.gridPosition.y})");
                        }
                        else
                        {
                            Debug.LogWarning($"物品对象 {placedItem.itemObject.name} 缺少InventorySystemItem组件");
                        }
                    }
                    else
                    {
                        Debug.LogWarning($"发现空的物品对象在位置 ({placedItem.position.x}, {placedItem.position.y})");
                    }
                }
                
                Debug.Log($"成功获取 {itemsData.Count} 个物品的保存数据");
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"获取网格物品数据时发生错误: {ex.Message}\n{ex.StackTrace}");
            }
        }
        else
        {
            Debug.LogWarning("尝试从空网格获取物品数据");
        }
    ...
    /// 验证当前网格数据（调试用）
    /// </summary>
    [ContextMenu("验证当前网格数据")]
    public void ValidateCurrentGridData()
    {
        Debug.Log("=== 开始验证当前网格数据 ===");
        
        if (currentBackpackGrid != null)
        {
            var backpackGrid = currentBackpackGrid.GetComponent<BackpackItemGrid>();
            if (backpackGrid != null)
            {
                var placedItems = backpackGrid.GetPlacedItems();
                Debug.Log($"当前背包网格状态:");
                Debug.Log($"- 网格尺寸: {backpackGrid.Width}x{backpackGrid.Height}");
                Debug.Log($"- 物品数量: {placedItems.Count}");
                Debug.Log($"- 网格激活状态: {currentBackpackGrid.activeInHierarchy}");
                
                var saveableGrid = backpackGrid as ISaveable;
                if (saveableGrid != null)
                {
                    Debug.Log($"- SaveID: {saveableGrid.GetSaveID()}");
                    Debug.Log($"- SaveID有效性: {saveableGrid.IsSaveIDValid()}");
                }
                ...
                foreach (var item in placedItems)
                {
                    ...
                    {
                        var itemComponent = item.itemObject.GetComponent<InventorySystemItem>();
                        if (itemComponent != null)
                        {
                            Debug.Log($"  物品: {itemComponent.Data.itemName} 位置: ({item.position.x}, {item.position.y})");
                        }
                    }
                }
            }
        ...
        if (currentTacticalRigGrid != null)
        {
            var tacticalRigGrid = currentTacticalRigGrid.GetComponent<TactiaclRigItemGrid>();
            if (tacticalRigGrid != null)
            {
                var placedItems = tacticalRigGrid.GetPlacedItems();
                Debug.Log($"当前战术挂具网格状态:");
                Debug.Log($"- 网格尺寸: {tacticalRigGrid.Width}x{tacticalRigGrid.Height}");
                Debug.Log($"- 物品数量: {placedItems.Count}");
                Debug.Log($"- 网格激活状态: {currentTacticalRigGrid.activeInHierarchy}");
            }
        }
        else
        {
    ...
    public void TestSaveFunction()
    {
        ...
        {
            SaveCurrentGridData(currentBackpackGrid, "Backpack");
            Debug.Log("背包数据保存测试完成");
        }
        
        if (currentTacticalRigGrid != null)
        {
            SaveCurrentGridData(currentTacticalRigGrid, "TacticalRig");
            Debug.Log("战术挂具数据保存测试完成");
        }
    ...
    private bool LoadDynamicGridData(GridSaveData gridData)
    {
        ...
        {
            Debug.LogWarning("网格数据为空，跳过恢复");
            return false; // 改为返回false表示恢复失败
        }
        ...
        try
        {
            ...
            {
                Debug.Log($"开始恢复背包网格数据，网格尺寸: {gridData.gridSize}, 物品数量: {(gridData.gridItems?.Count ?? 0)}");
                ...
                {
                    // 确保网格尺寸正确
                    if (gridData.gridSize != Vector2Int.zero)
                    {
                        Debug.Log($"设置背包网格尺寸: {gridData.gridSize.x}x{gridData.gridSize.y}");
                        // 注意：这里需要根据实际的背包数据来设置网格
                        // 如果有原始背包物品引用，应该调用SetBackpackData方法
                    }

                    // 恢复物品
                    if (gridData.gridItems != null && gridData.gridItems.Count > 0)
                    {
                        LoadGridItems(backpackGrid, gridData.gridItems);
                    }
                    else
                    {
                        Debug.Log("背包网格无物品数据需要恢复");
                    }
                }
                else
                {
                    Debug.LogError("创建的背包网格缺少BackpackItemGrid组件");
                    return false;
                }
            }
            else if (gridData.gridType == "TacticalRig" && tacticalRigGridPrefab != null && tacticalRigContainer != null)
            {
                Debug.Log($"开始恢复战术挂具网格数据，网格尺寸: {gridData.gridSize}, 物品数量: {(gridData.gridItems?.Count ?? 0)}");
                ...
                if (tacticalRigGrid != null)
                {
                    ...
                    if (gridData.gridItems != null && gridData.gridItems.Count > 0)
                    {
                        LoadGridItems(tacticalRigGrid, gridData.gridItems);
                    }
                    else
                    {
                        Debug.Log("战术挂具网格无物品数据需要恢复");
                    }
                }
                else
                {
                    Debug.LogError("创建的战术挂具网格缺少TactiaclRigItemGrid组件");
                    return false;
                }
            }
            else
            {
                Debug.LogWarning($"无法恢复网格类型: {gridData.gridType}，缺少必要的预制体或容器");
                return false;
            }

            Debug.Log($"成功恢复 {gridData.gridType} 网格数据");
            return true;
        }
        catch (System.Exception e)
        {
    ...
    private void LoadGridItems(BaseItemGrid grid, List<ItemSaveData> itemsData)
    {
        ...
        {
            Debug.LogError("物品恢复系统未初始化");
            return;
        }

        Debug.Log($"开始恢复 {itemsData.Count} 个物品到网格中");
        int successCount = 0;
        ...
        foreach (var itemData in itemsData)
        {
            ...
            try
            {
                var restoredItem = restorationSystem.RestoreItem(itemData, grid, itemData.gridPosition);
                if (restoredItem != null)
                {
                    successCount++;
                    Debug.Log($"成功恢复物品: {itemData.itemDataID} 到位置 ({itemData.gridPosition.x}, {itemData.gridPosition.y})");
                }
                else
                {
                    Debug.LogWarning($"恢复物品失败: {itemData.itemDataID}");
                }
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"恢复单个物品失败: {itemData.itemDataID}, 错误: {ex.Message}");
            }
        }

        Debug.Log($"物品恢复完成: 成功 {successCount}/{itemsData.Count} 个物品");
    }
    ...
    private void SaveCurrentGridData(GameObject gridObject, string gridType)
    {
        ...
        {
            // 在保存前先触发占用图谱保存（针对背包）
            if (gridType == "Backpack")
            {
                BackpackItemGrid backpackGrid = gridObject.GetComponent<BackpackItemGrid>();
                if (backpackGrid != null)
                {
                    // 触发背包占用图谱保存
                    Debug.Log($"EquipSlot {equipSlotID}: 触发背包占用图谱保存");
                    // 占用图谱会在BackpackItemGrid的GetSaveData方法中自动保存
                }
            }
            else if (gridType == "TacticalRig")
            {
                TactiaclRigItemGrid tacticalRigGrid = gridObject.GetComponent<TactiaclRigItemGrid>();
                if (tacticalRigGrid != null)
                {
                    // 触发战术挂具占用图谱保存（如果需要的话）
                    Debug.Log($"EquipSlot {equipSlotID}: 触发战术挂具占用图谱保存");
                    // 占用图谱会在TactiaclRigItemGrid的GetSaveData方法中自动保存
                }
            }
            ...
            if (!string.IsNullOrEmpty(gridID))
            {
                ...
                if (saveManager.IsObjectRegistered(saveableGrid))
                {
                    ...
                    Debug.Log($"EquipSlot {equipSlotID}: 已强制保存 {gridType} 网格数据和占用图谱，ID: {gridID}");
                }
                else
                {
