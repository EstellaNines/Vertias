...
namespace InventorySystem
{
    ...
    public class ContainerSaveManagerTester : MonoBehaviour
    {
        ...
        private System.Collections.IEnumerator RunBasicTestsCoroutine()
        {
            ...
            LogTest($"=== 测试完成，总体结果: {(allTestsPassed ? "✅ 通过" : "❌ 失败")} ===");
        }
        ...
        private bool TestSingletonInstance()
        {
            ...
            try
            {
                ...
                if (instance != null)
                {
                    LogTest("✅ ContainerSaveManager 单例实例创建成功");
                    return true;
                }
                else
                {
                    LogTest("❌ ContainerSaveManager 单例实例创建失败");
                    return false;
                }
            }
            catch (System.Exception e)
            {
                LogTest($"❌ 单例实例测试异常: {e.Message}");
                return false;
            }
        }
        ...
        private bool TestDataStructures()
        {
            ...
            try
            {
                ...
                {
                    LogTest("✅ ContainerSaveData 构造成功");
                }
                else
                {
                    LogTest("❌ ContainerSaveData 构造失败");
                    return false;
                }
                ...
                {
                    LogTest("✅ ContainerSaveDataCollection 构造成功");
                    return true;
                }
                else
                {
                    LogTest("❌ ContainerSaveDataCollection 构造失败");
                    return false;
                }
            }
            catch (System.Exception e)
            {
                LogTest($"❌ 数据结构测试异常: {e.Message}");
                return false;
            }
        }
        ...
        private bool TestEquipmentSlotIntegration()
        {
            ...
            {
                LogTest("⚠️ 场景中没有EquipmentSlot，跳过集成测试");
            ...
            LogTest($"✅ 找到 {containerSlotCount} 个容器类型装备槽");
            return true;
        }
        ...
        private System.Collections.IEnumerator TestItemDatabaseSystemCoroutine()
        {
            ...
            try
            {
                ...
                if (ItemDatabase.Instance.IsInitialized)
                {
                    LogTest($"✅ ItemDatabase已初始化，包含 {ItemDatabase.Instance.TotalItemCount} 个物品");
                }
                else
                {
                    LogTest("❌ ItemDatabase初始化失败");
                    databaseTestPassed = false;
                    LogTest($"✅ ItemDatabase系统测试完成");
                    LogTest($"=== 测试完成，总体结果: {(databaseTestPassed ? "✅ 通过" : "❌ 失败")} ===");
                    yield break;
                }
                ...
                if (item1402 != null)
                {
                    LogTest($"✅ 找到ID=1402的物品: {item1402.itemName} (类别: {item1402.category}, GlobalId: {item1402.GlobalId})");
                }
                else
                {
                    LogTest("❌ 未找到ID=1402的物品");
                    databaseTestPassed = false;
                }
                ...
                LogTest($"✅ ItemDatabase基础测试完成");
            }
            catch (System.Exception e)
            {
                LogTest($"❌ ItemDatabase系统测试失败: {e.Message}");
                databaseTestPassed = false;
                basicTestsPassed = false;
            }
        ...
        private System.Collections.IEnumerator TestItemCreation(ItemDataSO itemDataSO, System.Action<bool> onComplete)
        {
            ...
            try
            {
                ...
                {
                    LogTest($"⚠️ 无法访问LoadItemPrefab方法");
            ...
            if (testItem != null)
            {
                LogTest($"✅ 成功创建测试物品: {testItem.name}");
                ...
                {
                    LogTest($"✅ 物品组件完整: ItemDataReader + Item + DraggableItem + CanvasGroup");
