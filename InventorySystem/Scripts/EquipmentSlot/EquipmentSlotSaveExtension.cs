...
namespace InventorySystem
{
    ...
    public static class EquipmentSlotSaveExtension
    {
        ...
        public static void ApplyEquipmentSystemData(EquipmentSystemSaveData saveData)
        {
            ...
            foreach (var slotData in saveData.equipmentSlots)
            {
                ...
                if (slotData.hasEquippedItem && slotData.equippedItemData != null)
                {
                    ...
                    {
                        Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 装备恢复失败: {slotData.slotType}");
                    }
                }
            }
        ...
        private static bool RestoreEquippedItem(EquipmentSlotSaveData slotData)
        {
            ...
            if (slot == null)
            {
                Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 未找到类型为 {slotData.slotType} 的装备槽");
                return false;
            }
            ...
            {
                Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 无法创建物品实例: {slotData.equippedItemData.itemID}");
                return false;
            }
            ...
            {
                Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 物品实例缺少 ItemDataReader 组件");
            ...
            if (success)
            {
                ...
                {
                    Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 验证失败，槽位中没有装备");
                }
            ...
            {
                Debug.LogError($"[EquipmentSlotSaveExtension] ❌ 装备到槽位失败");
