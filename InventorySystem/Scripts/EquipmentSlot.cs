        ...
        if (itemRect != null)
        {
            ...
            if (textTransform != null)
            {
                ...
                if (textRect != null)
                {
                    ...
                    // 反向旋转TMP文字组件，使其保持正确方向
                    textTransform.localEulerAngles = new Vector3(0f, 0f, -itemRotation);
                    
                    // 重新计算文字位置，确保始终在视觉右下角
                    // 根据物品的旋转状态和装备槽的新尺寸重新定位文字
                    Vector2 textPosition;
                    bool isRotated = Mathf.Abs(itemRotation - 90f) < 1f; // 检查是否旋转了90度
        ...
        foreach (Image childImage in childImages)
        {
            if (childImage.gameObject != item) // 不调整自身
            {
                ...
                if (childRect != null)
                {
                    ...
                    // 根据子对象的轴心点计算正确的居中位置
                    // 对于轴心为(0.5,0.5)的子对象，居中坐标应该是(0, 0)
