...
public class GridMonitorDataManager
{
    ...
    private void InitializeDynamicDetection()
    {
        ...
        Debug.Log("[GridMonitorDataManager] 动态网格检测系统已初始化");
    }
    ...
    private void OnDynamicGridDetected(BaseItemGrid grid)
    {
        ...
        Debug.Log($"[GridMonitorDataManager] 检测到动态网格: {grid.name} (类型: {grid.GetType().Name})");
    ...
    {
        Debug.Log($"[GridMonitorDataManager] 动态网格被移除: {(grid != null ? grid.name : "null")}");
...
// 注意：GridMonitorSettings类已随静态监控系统一起移除